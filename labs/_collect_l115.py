...
s=json.loads((P/'_sources_l115.json').read_text());v=modal.Volume.from_name(s['volume']);sha=s['source_sha256'];missing=[];files={}
for preset,seeds in [('pilot',[100]),('paper',range(10))]:
 for seed in seeds:
  for name in ['identity.json','completed.json','result.json','predictions.npz','checkpoint.pt']+(['source_parity.json'] if preset=='pilot' else []):
   target=(P/'results/l115'/preset/f'seed-{seed}'/name) if name=='checkpoint.pt' else (P/'evidence/l115'/preset/f'seed-{seed}'/name)
...
(P/'evidence/l115/manifest.json').write_text(json.dumps({'source_sha256':sha,'files':files,'missing':missing},indent=2));print({'files':len(files),'missing':len(missing)})
