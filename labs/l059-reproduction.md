# L059 reproduction and evidence contract

Primary source: [Cawley and Talbot 2010](https://jmlr.org/papers/v11/cawley10a.html). [Lesson](../lessons/0059-validation-set-overfitting.html).
...
2. **Measured evidence:** `_verify_l059_results.json` contains the actual local run or frozen-result
...
```bash
.venv/bin/python labs/_run_foundation.py --lesson 59 --preset lab --output labs/data/cache/foundation/l059-rerun.json
...
```bash
modal run --detach modal/foundation_repro.py --lesson 59 --preset closer
```
