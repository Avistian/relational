...
r=json.loads((P/'_experiment_l100_results.json').read_text());assert r['status']=='COMPLETE' and len(r['runs'])==24
assert r['implementation_sha256']==hashlib.sha256((P/'relkit/checkpoint_l100.py').read_bytes()).hexdigest()
...
for x in r['runs']:
 assert len(x['trace'])==40 and [t['epoch'] for t in x['trace']]==list(range(1,41))
 best=min(x['trace'],key=lambda t:t['val_ce']);assert x['selected_epoch']==best['epoch'] and x['best_val_ce']==best['val_ce']
 path=P/'_experiment_l100_results-checkpoints'/x['checkpoint_file'];assert hashlib.sha256(path.read_bytes()).hexdigest()==x['checkpoint_sha256']
...
for seed in m.SEEDS:
 hashes=[x['sample_hashes'] for x in r['runs'] if x['seed']==seed]
 assert len(hashes)==8 and all(h==hashes[0] for h in hashes)
 assert all(t['updates']==4 for x in r['runs'] for t in x['trace'])
for arm in m.ARMS:
...
report={'status':'PASS','complete_fits':24,'complete_epochs':960,'restored_checkpoints':24,'selected_prediction_sets':12,'independent_sklearn_metrics':'EXACT','validation_only_selection':'EXACT','raw_data_and_reverse_edges':'PASS','paired_sample_hashes':'EXACT','complete_optimizer_updates':3840,'paper_parity':'NOT_ESTABLISHED'}
(P/'_audit_l100_results.json').write_text(json.dumps(report,indent=2)+'\n');print(report)
