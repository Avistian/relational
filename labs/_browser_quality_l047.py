"""Chromium visual-state check for the individual L047 quality audit."""
import functools,json,threading
from pathlib import Path
from http.server import SimpleHTTPRequestHandler,ThreadingHTTPServer
from playwright.sync_api import sync_playwright
ROOT=Path(__file__).resolve().parents[1]
OUT=Path('/tmp/quality-l047-browser');OUT.mkdir(exist_ok=True)
...
with sync_playwright() as p:
    ...
    for width in [900,375]:
        ...
        page.goto(base+'/lessons/0047-saint.html',wait_until='networkidle')
        ...
        for name in ['axes','architecture','context','views','contrastive','cd']:
        ...
        page.locator('#architecture-saint').screenshot(path=str(OUT/f'{width}-atlas.png'))
        page.close()
    browser.close()
server.shutdown()
report=dict(status='PASS' if not errors and not missing and not problems else 'FAIL',errors=errors,missing=missing,problems=problems,records=records,screenshots=str(OUT),scope='Live local Chromium; six visual widgets, all native buttons, slider endpoints and keyboard at 900/375; not live Colab or deployed site')
(ROOT/'reviews/lesson-quality-audit-047-070/047-browser.json').write_text(json.dumps(report,indent=2)+'\n')
