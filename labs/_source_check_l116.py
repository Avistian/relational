...
import torch
from relkit.debug_l116 import GCN,normalized_adjacency,release_state
P=Path(__file__).resolve().parent

def original():
    sys.path.insert(0,str(P/'sources/l116'))
    spec=importlib.util.spec_from_file_location('ogb_original',P/'sources/l116/gnn.py');m=importlib.util.module_from_spec(spec);spec.loader.exec_module(m);return m.GCN

def transfer(model,ref):
    state=release_state(model);converted={}
...
def check(device='cpu'):
    ...
    for training in [False,True]:
        ...
        for name,p in model.named_parameters():
            name=name.replace('processor.','').replace('head.conv.','convs.2.')
...
if __name__=='__main__':
    r=check(sys.argv[1] if len(sys.argv)>1 else 'cpu');(P/'_source_check_l116_results.json').write_text(json.dumps(r,indent=2));print(r)
