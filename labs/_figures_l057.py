...
path=ROOT/'_verify_l057_results.json'
if path.exists():
 ...
 fig.suptitle('Measured: three model seeds; bars = sample SD (fixed rows/folds)',fontsize=13);fig.tight_layout();save(fig,'results')
 ...
 fig.tight_layout();save(fig,'uncertainty')
 ...
 table+='</tbody></table>'
 block='<details><summary>Reveal the measured three-family results</summary><p>Author run: three real datasets × three seeds × three OOF folds × three families. Loss means ± sample SD; all family and combination scores use identical outer-test rows. '+f'Elapsed {r["seconds"]:.1f} CPU wall seconds. INCOMPARABLE to the paper.</p><div class="ensemble-scroll">'+table+'</div><figure tabindex="0"><img class="ensemble-figure" src="../labs/figures/l057/results.png" alt="Measured per-seed test log loss for each family and combiner"><figcaption>Points show every seed; bars are sample SD, not confidence intervals. Independent y-axis zooms expose within-dataset differences.</figcaption></figure><figure tabindex="0"><img class="ensemble-figure" src="../labs/figures/l057/uncertainty.png" alt="Paired test loss gaps with conditional seed intervals and mean ranks with critical difference"><figcaption>Dataset means are ranked after seed averaging. Three datasets provide little power.</figcaption></figure></details>'
 import re
