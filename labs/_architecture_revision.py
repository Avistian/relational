...
panel(49,'trompt','Trompt · prompts gather feature evidence','Model architecture — Trompt',
      'Which dimension disappears when each prompt gathers a row’s feature values?',
      [('Encode columns and values','Column identities and cell values enter the model through distinct representations.','column C × d; cells B × C × d'),
       ('Route prompts','Prompt/row-conditioned states score column identities; normalize over C.','B × P × C'),
       ('Gather and iterate','Weight expanded feature values and sum over columns; full model repeats prompt processing.','B × P × C × d → B × P × d'),
       ('Read prompt states','The full model combines prompt states for its prediction head.','row representation → output')],
      'One prompt gathers three vectors',
      matrix(['col 1','col 2','col 3'],[('weight',['0.2','0.3','0.5']),('value 1',['1','0','4']),('value 2',['0','2','1'])])+eq('0.2(1,0) + 0.3(0,2) + 0.5(4,1)')+op('<span>sum over C</span>',vec([2.2,1.1])),
      'Illustrative weights after column softmax. Prompts are learned vectors, not natural-language instructions.',
      'Aggregation removes the column axis C while preserving prompt P and hidden coordinate d.',
      'Paper-level route; the local lab implements prompt weighting and aggregation only. Full Trompt training and benchmark reproduction are not implemented here.')
