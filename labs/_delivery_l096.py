...
R=Path(__file__).resolve().parents[1];P=R/'labs';S='0096-multi-relational-data'
...
with sync_playwright() as pw:
 ...
 for width in [1200,375]:
  ...
  widget=page.locator('#schema-intervention');select=widget.locator('select');out=widget.locator('output')
  assert '10 = 5 units' in out.inner_text()
  select.select_option('50');assert '10 = 2 units' in out.inner_text() and '50 = 4 units' in out.inner_text()
  widget.locator('button').click();assert select.input_value()=='10'
...
(P/'_delivery_l096_results.json').write_text('{"status":"RUNNING"}\n')
...
with tempfile.TemporaryDirectory(prefix='l096-pages-') as tmp:
 ...
 checked=0
 for path in [stage/'lessons'/f'{S}.html',stage/'reference/schema-graph-contract.html']:
 ...
 for relative in ['labs/relkit/schema_l096.py','labs/_run_l096.py','labs/l096-reproduction.md','labs/_experiment_l096_results.json','labs/solutions/'+S+'.ipynb']:
