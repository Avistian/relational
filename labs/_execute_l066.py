"""Execute only the local ignored L066 teacher; preserve standard rich outputs."""
import os,time,json
os.environ['OMP_NUM_THREADS']='1';os.environ['OPENBLAS_NUM_THREADS']='1'
os.environ.setdefault('MPLCONFIGDIR','/tmp/l066-matplotlib');os.environ.setdefault('IPYTHONDIR','/tmp/l066-ipython')
from pathlib import Path
import nbformat
from IPython.terminal.interactiveshell import TerminalInteractiveShell
from IPython.utils.capture import capture_output
root=Path(__file__).resolve().parent;path=root/'solutions/0066-tabicl-column-row-attention.ipynb'
nb=nbformat.read(path,as_version=4);shell=TerminalInteractiveShell.instance();count=0;start=time.perf_counter()
os.environ['L066_OUTPUT']=str(root/'data/cache/l066-teacher'/str(time.time_ns()))
os.chdir(root)
...
report=dict(status='PASS',code_cells=count,seconds=time.perf_counter()-start,scope='Five live model tasks, original copied-weight full-stage/source/wrapper parity, nested-helper and weight stale-evidence rejection, full diabetes nested context measurements, author numerical correspondence and fresh EXIT; broader gate off')
(root/'_execution_l066_v2_results.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report))
