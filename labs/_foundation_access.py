...
70:'Implement loss, validation selection, complete panels, paired dataset resampling and lifecycle costs; fit corrected TabM and XGBoost candidates, then measure a frozen feature-erasure intervention.'}
EVIDENCE={
...
70:'Corrected seven-arm hybrid panel: six archived procedures plus 15 fresh corrected TabM fits, with 15 additional fresh XGBoost controls and 30 measured interventions; original pretrained predictions independently replayed.'}
...
def lab_plan(n):
    ...
    if n == 70:exit_path = 'data/cache/l070-student/<run-id>/exit-v2.json'
...
def build_directory():
    ...
    for n in range(58,71):
        ...
        if n == 70:
            links[-1] = link('../_verify_l070_results.json','Immutable historical seven-arm results')
            links.append(link('../_verify_l070_v2_results.json','Corrected fresh fits and hybrid comparison'))
        if n == 69:
        ...
        if n == 66:
            links.append(link('../_verify_l066_v2_results.json','Original TabICL context results'))
        if n == 65:
