# L068 reproduction and evidence contract

Primary source: [Drift-Resilient TabPFN](https://arxiv.org/html/2411.10634v1). [Lesson](../lessons/0068-pfns-under-temporal-shift.html).
...
2. **Measured evidence:** `_verify_l068_results.json` contains the actual local run or frozen-result
...
```bash
.venv/bin/python labs/_run_foundation.py --lesson 68 --preset lab --output labs/data/cache/foundation/l068-rerun.json
.venv/bin/python labs/_build_l068.py
```
...
```bash
modal run --detach modal/foundation_repro.py --lesson 68 --preset closer
```
