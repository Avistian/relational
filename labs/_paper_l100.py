...
if args.target=='aifb':
 ...
 r['scope']='Fresh L100 replay of complete AIFB release-protocol port; not the ACM comparison'
 r['implementation_sha256']=hashlib.sha256((P/'relkit/rgcn_l091.py').read_bytes()).hexdigest()
 (P/'_paper_l100_aifb_results.json').write_text(json.dumps(r,indent=2)+'\n');print(r['mean'],r['sample_sd'])
else:
 m=json.loads((P/'_sources_l093.json').read_text())
 subprocess.run([sys.executable,str(P/'_run_l093.py'),'--preset','paper','--data',str(P/'data/l093/graph_CS.pk'),'--sha256',m['cs_data']['sha256'],'--seeds','0','1','2','3','4','--arms',*(['hgt','rgcn'] if args.with_rgcn else ['hgt']),'--device',args.device,'--output',str(P/'_paper_l100_hgt_cs_results.json')],check=True)
