"""Execute the ignored L069 teacher and bind its fresh EXIT and exact files."""
import os,time,json,hashlib
os.environ['OMP_NUM_THREADS']='1';os.environ['OPENBLAS_NUM_THREADS']='1'
os.environ.setdefault('MPLCONFIGDIR','/tmp/l069-matplotlib');os.environ.setdefault('IPYTHONDIR','/tmp/l069-ipython')
from pathlib import Path
import nbformat
from IPython.terminal.interactiveshell import TerminalInteractiveShell
from IPython.utils.capture import capture_output
root=Path(__file__).resolve().parent;path=root/'solutions/0069-tabpfn-open-environment-failures.ipynb';input_sha=hashlib.sha256(path.read_bytes()).hexdigest();operator_sha=hashlib.sha256(Path(__file__).read_bytes()).hexdigest()
...
for cell in nb.cells:
 ...
 if cell.source.startswith('# CHECK — exact task'):
  ...
  while not (root/'_verify_l069_v2_results.json').exists():time.sleep(1)
