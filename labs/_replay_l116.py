...
import numpy as np
import torch
from relkit.debug_l116 import GCN,load_arxiv,normalized_adjacency,sha256
from _source_check_l116 import original,transfer
from _sparse_input_l116 import binary_for_reference
...
with torch.no_grad():
 for seed in range(10):
  d=P/'evidence/l116/paper'/f'seed-{seed}';cp=P/'results/l116/paper'/f'seed-{seed}'/'checkpoint.pt';record=json.loads((d/'result.json').read_text());assert sha256(cp)==record['checkpoint_sha256']
...
r={'status':'PASS','runs':rows,'nodes_replayed':len(y)*10,'seconds':time.perf_counter()-start,'scope':'All final predictions via original GCN class and GCNConv, native CSR input; current CPU runtime','historical_identity':'NOT_ESTABLISHED'};(P/'_replay_l116_results.json').write_text(json.dumps(r,indent=2))
