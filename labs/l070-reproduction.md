# L070 reproduction and evidence contract

Primary source: [TabPFN-3 technical report and historical controls](https://arxiv.org/abs/2605.13986). [Lesson](../lessons/0070-foundation-model-checkpoint.html).
...
2. **Measured evidence:** `_verify_l070_results.json` contains the actual local run or frozen-result
...
```bash
.venv/bin/python labs/_run_foundation.py --lesson 70 --preset lab --output labs/data/cache/foundation/l070-rerun.json
...
```bash
modal run --detach modal/foundation_repro.py --lesson 70 --preset closer
```
