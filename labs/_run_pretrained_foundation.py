...
if __name__=='__main__':
    from torch import nn
    parser=argparse.ArgumentParser();parser.add_argument('--lesson',type=int,required=True);parser.add_argument('--output');args=parser.parse_args()
    ...
    output=Path(args.output) if args.output else ROOT/f'_verify_l{args.lesson:03}_results.json'
    output.parent.mkdir(parents=True,exist_ok=True)
    output.write_text(json.dumps(result,indent=2,allow_nan=False,default=lambda x:x.item())+'\n')
