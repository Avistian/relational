# L061 reproduction and evidence contract

Primary source: [Transformers Can Do Bayesian Inference](https://arxiv.org/html/2112.10510v7). [Lesson](../lessons/0061-prior-data-fitted-networks.html).
...
2. **Measured evidence:** `_verify_l061_results.json` contains the actual local run or frozen-result
...
```bash
.venv/bin/python labs/_run_foundation.py --lesson 61 --preset lab --output labs/data/cache/foundation/l061-rerun.json
...
```bash
modal run --detach modal/foundation_repro.py --lesson 61 --preset closer
```
