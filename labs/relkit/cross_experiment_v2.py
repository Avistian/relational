"""L057 v2 OOF cross-family stack; corrected TabM-mini dependency. Fixed recipes avoid held-out early-stopping leakage."""
...
def family_ablation(oof, test, y, yt, families, steps=40):
    """Remove each family and RESELECT on OOF; score the frozen removal on test.

    test_gap = loss(without family) - loss(full stack): positive favors inclusion.
    The original weights are not simply renormalized. This is a conditional
    library ablation, not a causal attribution or a fresh confirmatory test.
    """
    if len(families)<2:raise ValueError('Need at least two families')
    full=evaluate_library(oof,test,y,yt,families,steps)
    rows=[]
    for removed,name in enumerate(families):
        keep=[j for j in range(len(families)) if j!=removed]
        reduced=evaluate_library(oof[:,keep],test[:,keep],y,yt,[families[j] for j in keep],steps)
        weights=np.zeros(len(families));weights[keep]=reduced['weights']
        rows.append(dict(removed=name,removed_index=removed,weights=weights.tolist(),
            selected=reduced['selected'],original_weight=full['weights'][removed],
            test_loss=reduced['errors']['Greedy-stack'],
            test_gap=reduced['errors']['Greedy-stack']-full['errors']['Greedy-stack'],
            oof_loss=binary_loss(y,blend(oof,weights))))
    return rows
