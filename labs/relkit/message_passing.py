"""L078: visible mean messages and complete Cora GCN release-protocol port.
...
def mean_neighbors(x, edges):
    """Edges contain (source, destination); an empty neighborhood has mean zero."""
    x=np.asarray(x,dtype=float);edges=np.asarray(edges,dtype=int).reshape(-1,2)
    sums=np.zeros_like(x);counts=np.zeros(len(x))
    for source,destination in edges:
        sums[destination]+=x[source];counts[destination]+=1
    return sums/np.maximum(counts,1)[:,None]


def mean_step(x, edges):
    """Synchronous illustrative update; no learned weights in this fixture."""
    return np.maximum(0,.5*np.asarray(x)+.5*mean_neighbors(x,edges))


def gcn_support(a):
    """Dense teaching form; binary undirected adjacency without self-edges."""
    a=np.asarray(a,dtype=float)
    if not np.allclose(a,a.T) or np.any(np.diag(a)):
        raise ValueError('Expected undirected adjacency without self-loops')
    t=a+np.eye(len(a));d=t.sum(1)**-.5
    return d[:,None]*t*d[None,:]
