...
from relkit.tabpfn_l069_v2 import TabPFNv2,MODEL_CONFIG,CHECKPOINT_SHA,ensure_checkpoint,load_pretrained,predict_numeric
...
def dataset_summary(records):
    ...
    for key,rows in groups.items():
        ...
        for metric in metrics:
            ...
            if means:result.update({metric+'_n':len(means),metric+'_seed_ids':[seed for seed in result['seeds'] if any(r['seed']==seed and r.get(metric) is not None for r in rows)],metric+'_mean':float(np.mean(means)),metric+'_sd':float(np.std(means,ddof=1)) if len(means)>1 else None,metric+'_seed_values':means})
        out.append(result)
    return out
...
def metric_record(y,p,classes):
    ...
    if r['unsupported_n']==0 and len(np.unique(y))==len(classes):
...
def stable_code(obj,seen=None):
    ...
    if isinstance(obj,types.FunctionType):
        ...
        visit(obj.__code__)
        dependencies={n:stable_code(obj.__globals__[n],seen) for n in sorted(names) if n in obj.__globals__ and (isinstance(obj.__globals__[n],(types.FunctionType,type)) and (obj.__globals__[n].__module__==obj.__module__ or obj.__globals__[n].__module__.startswith('relkit.')))}
...
def model_runtime_identity(model):
    ...
    for name,module in model.named_modules():
        if 'forward' in vars(module) or module._forward_hooks or module._forward_pre_hooks or module._backward_hooks:raise RuntimeError('Hooks and instance forward overrides invalidate certification')
        state[name]=dict(type=type(module).__qualname__,forward=stable_code(module.forward.__func__),settings=stable_code({k:v for k,v in vars(module).items() if not k.startswith('_')}))
    return hashlib.sha256(json.dumps(state,sort_keys=True).encode()).hexdigest()

def align_schema(query,available_names,training_names):
    """Declared incremental policy: ignore new columns, reject missing trained names."""
    if len(set(available_names))!=len(available_names) or len(set(training_names))!=len(training_names):raise ValueError('Duplicate column identity')
    if any(n not in available_names for n in training_names):raise ValueError('Missing trained feature: choose a fallback explicitly')
    return np.asarray(query)[:,[available_names.index(n) for n in training_names]].copy()


def kernel_identity(namespace,root):
    names=['novel_split','novelty_scores','impute_features','all_row_metrics','dataset_summary','shift_task','load_dataset','metric_record','run_experiment','predict_numeric','TabPFNv2','load_pretrained','ensure_checkpoint','model_digest','model_runtime_identity','align_schema']
    payload=dict(xgboost_methods={k:stable_code(getattr(namespace['XGBClassifier'],k)) for k in ['fit','predict_proba','__init__']},functions={n:stable_code(namespace[n]) for n in names},protocol=PROTOCOL,data=DATA_SHA,checkpoint=CHECKPOINT_SHA,model_source=hashlib.sha256((Path(root)/'relkit/tabpfn_l069_v2.py').read_bytes()).hexdigest(),versions={k:importlib.metadata.version(k) for k in ['numpy','pandas','torch','scikit-learn','xgboost']})
...
def run_experiment(root,config=None,namespace=None):
    ...
    model,_=s['load_pretrained'](s['ensure_checkpoint'](root),s['TabPFNv2'](**MODEL_CONFIG));records=[];datasets={};splits=[];model_identity=dict(weights=s['model_digest'](model),runtime=s['model_runtime_identity'](model),parameters=sum(p.numel() for p in model.parameters()),tensors=len(model.state_dict()));incremental_controls=[]
        ...
        for dataset in cfg[axis]:
            for seed in cfg['seeds']:
                ...
                if axis=='novelty':
                    ...
                    names=metadata['features'];augmented=np.column_stack([x[query],np.linalg.norm(x[query],axis=1)])
                    projected=s['align_schema'](augmented,names+['new_sensor'],names);assert np.array_equal(projected,x[query])
                    incremental_controls.append(dict(dataset=dataset,seed=seed,policy='ignore unseen column by training schema',input_max_delta=float(np.max(abs(projected-x[query]))),ignored_columns=['new_sensor']))
    ...
    assert model_identity['weights']==s['model_digest'](model) and model_identity['runtime']==s['model_runtime_identity'](model),'Actual model state changed'
    if s['kernel_identity'](s,root)['sha256']!=identity['sha256']:raise RuntimeError('Live code changed during measurement')
    return dict(schema='l069-v2',model_identity=model_identity,incremental_controls=incremental_controls,config=cfg,protocol=PROTOCOL,kernel_identity=identity,source_commit=SOURCE_COMMIT,datasets=datasets,splits=splits,records=records,summary=s['dataset_summary'](records),seconds=time.perf_counter()-start,paper_reproduction='INCOMPARABLE: local diagnostic; original tuned seven-model four-axis benchmark NOT_RUN')
