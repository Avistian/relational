"""L069: full historical default-classifier TabPFN v2 computation, visible and uncached.
...
def attention_mix(q,k,v):
    ...
    shape=q.shape
    q=q.reshape(-1,*shape[-3:])
    k=k.expand(*shape[:-3],shape[-3],k.shape[-2],k.shape[-1]).reshape(-1,shape[-3],k.shape[-2],k.shape[-1])
    v=v.expand(*shape[:-3],shape[-3],v.shape[-2],v.shape[-1]).reshape(-1,shape[-3],v.shape[-2],v.shape[-1])
    return nn.functional.scaled_dot_product_attention(q,k,v,dropout_p=0.,scale=float(scale)).reshape(*shape[:-1],v.shape[-1])
