...
def pair_features(z,pairs,directed=False):
    """Undirected: elementwise product. Directed: ordered concatenation."""
    source,target=z[pairs[0]],z[pairs[1]]
    return torch.cat([source,target],dim=-1) if directed else source*target

# %% Task 3: pool within graph boundaries

def graph_mean(z,batch,num_graphs):
    """Per-graph means; an explicitly requested empty graph has zero vector."""
    total=z.new_zeros((num_graphs,z.shape[1]))
    total.index_add_(0,batch,z)
    count=torch.bincount(batch,minlength=num_graphs).to(z.dtype).clamp_min(1)
    return total/count[:,None]

# %% Train on authorized labels
