"""Visible one-layer TGN-attn and checkpoint experiment for L110 (Rossi et al., 2020).
...
# %% Task 1: restore one complete selected checkpoint
...
# %% Task 2: keep timestamp ties within one scoring batch
...
# %% Task 3: distinguish occurrence from knowledge
...
# %% Snapshot selection includes queued messages

def capture_checkpoint(model):
    return {'weights': {k: v.detach().clone() for k, v in model.state_dict().items()
...
def evaluate(model, events, sampler, batch_size=200, clean=False):
    model.eval(); sampler.reset(); aps=[]; aucs=[]; saved=[]
    with torch.no_grad():
        for b in (strict_batches if clean else batches)(events,batch_size):
            ...
            saved.append({'batch_id':np.full(len(b['e']),len(saved),dtype=np.int64),'edges':b['e'],'negative':neg,'positive':positive.cpu().numpy(),'negative_score':negative.cpu().numpy()})
    return {'ap':float(np.mean(aps)),'auc':float(np.mean(aucs)),'batch_ap':aps,'batch_auc':aucs},saved


def run_training(nodes,edges,data,seed=0,epochs=50,patience=5,batch_size=200,output=None,device='cpu',arm='release'):
    """Paired protocols: preserve release, or restore atomic best state with tie-safe batches."""
    assert arm in ['release','clean']
    clean = arm == 'clean'
    ...
    for epoch in range(epochs):
        ...
        for b in (strict_batches if clean else batches)(train,batch_size):
            optimizer.zero_grad()
        ...
        val,_=evaluate(model,data['val'],val_sampler,batch_size,clean)
        ...
        new_val,_=evaluate(model,data['new_val'],val_sampler,batch_size,clean)
        ...
        if improved:
            ...
            best_weights={k:v.detach().clone() for k,v in model.state_dict().items()
                          if k not in ['node_features','edge_features']}
            best_checkpoint=capture_checkpoint(model)
        else: rounds+=1
    ...
    if clean:
        restore_checkpoint(model,best_checkpoint)
    selected=best_epoch if (stop or clean) else len(trace)-1
    val_state=model.snapshot()
    test,predictions=evaluate(model,data['test'],test_sampler,batch_size,clean)
    model.restore(val_state)
    new_test,new_predictions=evaluate(model,data['new_test'],new_sampler,batch_size,clean)
    model.restore(val_state)
    result={'arm':arm,'seed':seed,'epochs_completed':len(trace),'selected_epoch':selected,'early_stopped':stop,
            ...
            'checkpoint_policy':('clean: best validation weights and complete temporal state; timestamp ties grouped' if clean else 'release: best state_dict on early stop; pending messages from stopping epoch')}
    if output:
        out=Path(output)
