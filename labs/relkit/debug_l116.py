...
"""L116: instrumented GCN training and falsifiable diagnostic probes.
Architecture follows OGB's MIT-licensed gnn.py; see labs/sources/l116/LICENSE.
"""
import copy
import hashlib
import json
...
# %% Preserve the complete GCN forward pass

def graph_forward(encoder,processor,head,x,adj):
    """The head receives adjacency because some task heads propagate too."""
    return head(processor(encoder(x),adj),adj)

# %% Modular GCN: identity encoder, hidden processor, propagating head
...
# %% Task 1: isolate authorized training labels

def training_loss(log_probs, labels, train_idx):
    return F.nll_loss(log_probs[train_idx],labels[train_idx])

# %% Task 2: repair the training update

def train_step(model,optimizer,x,adj,y,train_idx):
    """One supervised update; evaluation callers may have left model in eval mode."""
    model.train()
    optimizer.zero_grad()
...
# %% Task 3: align seed outputs with global labels

def seed_loss(log_probs,global_labels,n_id,batch_size):
    """NeighborLoader convention: first batch_size rows are input seed nodes.
    Caller must ensure these seeds are authorized training nodes. Context nodes
    support messages but do not become additional supervised examples.
    """
    return F.nll_loss(log_probs[:batch_size],global_labels[n_id[:batch_size]])

# %% Intentionally faulty course intervention

def missing_step(model,optimizer,x,adj,y,train_idx):
    """Injected bug for the paired course trace; never the paper training path."""
    model.train();optimizer.zero_grad()
    loss=training_loss(model(x,adj),y,train_idx);loss.backward()
    return loss.detach()

def vector_norm(tensors):
    values=[t.detach().square().sum() for t in tensors if t is not None]
...
def train_run(x,adj,y,split,seed,epochs,output,device='cpu',fault=False):
    output=Path(output)
    ...
    for epoch in range(epochs):
        ...
        loss=(missing_step if fault else train_step)(model,optimizer,x,adj,y,split['train'])
        grad_norm=vector_norm(p.grad for p in model.parameters())
        update_norm=vector_norm(p.detach()-b for p,b in zip(model.parameters(),before))
        if not math.isfinite(float(loss)) or not math.isfinite(grad_norm):
            raise FloatingPointError('Non-finite loss/gradient; stop and diagnose')
        scores,_=evaluate(model,x,adj,y,split)
        history.append({'epoch':epoch+1,'loss':float(loss),'gradient_l2':grad_norm,'update_l2':update_norm,**scores})
    ...
    result={'seed':seed,'epochs':epochs,'selected_epoch':chosen+1,'scores':scores,'seconds':time.perf_counter()-start,'parameters':sum(p.numel() for p in model.parameters()),'history':history,'checkpoint_sha256':sha256(output/'checkpoint.pt'),'predictions_sha256':sha256(output/'predictions.npz'),'device':device,'torch':torch.__version__,'numpy':np.__version__,'fault_missing_step':fault,'status':'COMPLETE' if epochs==500 and not fault else 'COURSE_INTERVENTION'}
