"""Run the measured L054 evidence and write _verify_l054_v2_results.json.
...
from relkit.tabm_experiment_v2 import run_suite, _standardise, fit_tabm
...
if __name__ == '__main__':
    ...
    import hashlib
    result['source_hashes'] = {str(p.relative_to(ROOT)): hashlib.sha256(p.read_bytes()).hexdigest() for p in [ROOT/'relkit/tabm_v2.py', ROOT/'relkit/tabm_experiment_v2.py', ROOT/'relkit/realmlp_experiment.py', ROOT/'_verify_l054_v2.py']}
    result['track'] = 'corrected-numeric-mini-v2'
    result['total_seconds'] = time.time() - start
    (ROOT / '_verify_l054_v2_results.json').write_text(json.dumps(result, indent=1))
    print('WROTE _verify_l054_v2_results.json in', round(result['total_seconds'], 1), 's')
