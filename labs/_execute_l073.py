...
ROOT=Path(__file__).resolve().parent;SLUG='0073-when-ssl-helps'
if __name__=='__main__':
    ...
    nbformat.write(nb,p)
    actual=json.loads((ROOT/'l073-student-results.json').read_text());expected=json.loads((ROOT/'_verify_l073_results.json').read_text())
    ...
    report={'status':'PASS','code_cells':sum(c.cell_type=='code' for c in nb.cells),'evaluations':len(actual['records']),
            'exact_record_parity':True,'implementation_sha256':hashlib.sha256((ROOT/'relkit/ssl_regimes_l073.py').read_bytes()).hexdigest(),
            'live_colab':'NOT_CHECKED','larger_run':'NOT_RUN'}
    (ROOT/'_execution_l073_results.json').write_text(json.dumps(report,indent=2));(ROOT/'l073-student-results.json').unlink();print(report)
