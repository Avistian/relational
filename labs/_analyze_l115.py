...
import numpy as np
P=Path(__file__).resolve().parent;E=P/'evidence/l115';sources=json.loads((P/'_sources_l115.json').read_text());audit=json.loads((P/'_audit_l115_results.json').read_text());rows=[];cost=0
...
(E/'summary.json').write_text(json.dumps(out,indent=2));b=json.loads((P/'_budget_l115.json').read_text());b.update(status='COMPLETE',successful_resource_usd=cost);(P/'_budget_l115.json').write_text(json.dumps(b,indent=2));print(out)
