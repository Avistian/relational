...
('select_krr','def select_krr(x, y, candidates):','Select by mean PRESS across candidates','Fit each declared configuration using krr_fit, compute mean squared live loo_residuals, choose with live choose_validation, and return (chosen_model, index, all_losses). Reject an empty list.','The final candidate search must use your deleted residuals; displaying a LOO example alone cannot validate a different training loop.','Keep model and score ordering aligned. Candidate values are regularization and eta.'),
