...
**Environment:** bounded CPU training, with no pretrained checkpoint download on the default path. Colab bootstrap supplies code and data access; exact public data snapshots are bundled and verified before use. The entire recap below is self-contained. **Workflow:** attempt retrieval and prediction; implement the TODO; diagnose with CHECK; execute the live experiment; then defend the EXIT. Figures and tables labeled author evidence are not your current kernel's results. Full original pretraining/benchmark reproduction is INCOMPARABLE/NOT_RUN.''')
