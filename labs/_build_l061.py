...
def build_package(notebooks=True,render=True):
 ...
 cfg=dict(lesson=61,mode='pfn',answer='Because every context and query row is drawn from one shared sampled task, the labeled context is genuinely informative about the query. Minimizing the expected held-out NLL drives the network toward the posterior predictive distribution (PPD). Finite weights, a finite number of density bins, and prior mismatch keep it short of the exact answer.',quiz=['Can a query attend to itself without target leakage?',['Yes, if its target is never encoded','No, every self edge reveals a target','Yes, if its target is shuffled first'],0,'A query token holds features only; its target is never encoded, so reading itself reveals no label. Query self edges are therefore legal, while edges to other query rows stay blocked at every layer.'])
...
What is checked, and what remains out of scope. The original row Transformer computation and the full-support density are validated with copied nonzero weights and gradients. The local GP is one-dimensional, with width 64, 3 blocks, 64 bins, and 2000 updates per seed. It learns useful conditioning but does not match the exact posterior. A separate oracle puts analytic GP masses into the same fixed head, which reveals the finite-head approximation error on its own.
