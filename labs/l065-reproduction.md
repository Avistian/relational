# L065 reproduction and evidence contract

Primary source: [A Closer Look at TabPFN v2, Section 6](https://arxiv.org/html/2502.17361v1). [Lesson](../lessons/0065-tabpfn-query-embeddings.html).
...
2. **Measured evidence:** `_verify_l065_results.json` contains the actual local run or frozen-result
...
```bash
.venv/bin/python labs/_run_foundation.py --lesson 65 --preset lab --output labs/data/cache/foundation/l065-rerun.json
...
```bash
modal run --detach modal/foundation_repro.py --lesson 65 --preset closer
```
