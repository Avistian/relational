"""Execute only the local ignored L061 teacher; preserve standard rich outputs."""
import os,time,json
os.environ['OMP_NUM_THREADS']='1';os.environ['OPENBLAS_NUM_THREADS']='1'
os.environ.setdefault('MPLCONFIGDIR','/tmp/l061-matplotlib');os.environ.setdefault('IPYTHONDIR','/tmp/l061-ipython')
from pathlib import Path
import nbformat
from IPython.terminal.interactiveshell import TerminalInteractiveShell
from IPython.utils.capture import capture_output
root=Path(__file__).resolve().parent;path=root/'solutions/0061-prior-data-fitted-networks.ipynb'
...
report=dict(status='PASS',code_cells=count,seconds=time.perf_counter()-start,scope='Five live PFN TODOs, original source parity, generator negative control, kernel mutation checks, fresh smoke plus2000step seed7 learned fit and concrete EXIT; scale-up gate off')
(root/'_execution_l061_v2_results.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report))
