"""Source parity, live-helper mutation checks and the real copied Pages tree."""
import ast,base64,hashlib,json,re,subprocess,tempfile
from pathlib import Path
from urllib.parse import urlsplit,unquote
import nbformat
from bs4 import BeautifulSoup
ROOT=Path(__file__).resolve().parents[1];LAB=ROOT/'labs';SLUG='0077-single-table-ceiling'
def run():
    ...
    source=(LAB/'relkit/ceiling_l077.py').read_text()
    expected={n.name:ast.dump(n,include_attributes=False) for n in ast.parse(source).body if isinstance(n,ast.FunctionDef)}
    actual={}
    ...
    # Execute actual inline definitions in a fresh namespace, then corrupt each TODO.
    env={}
    for c in teacher.cells:
        if c.cell_type!='code' or '@colab-bootstrap' in c.source:continue
    ...
    baseline=env['run_experiment'](30,7)
    mutations=[]
    for name,replacement in [
        ('temporal_delta',lambda c,e,t:env['np'].zeros(len(c))),
        ('flat_rows',lambda c,e,t:env['np'].array([[x['target']] for x in c])),
        ('collision_ceiling',lambda x,y:1.)]:
        saved=env[name];env[name]=replacement
        result=env['run_experiment'](30,7);env[name]=saved
        assert result!=baseline,name+' is not live'
        assert result['scores']['flat']['test']!=.5 or result['scores']['restored']['test']!=1. or result['ceilings']['flat']!=.5,name+' escaped contract'
        mutations.append(name)
    digest=hashlib.sha256(source.encode()).hexdigest()
    for name in ['_execution_l077_results.json','_verify_l077_results.json']:
        r=json.loads((LAB/name).read_text());assert r['status']=='PASS' and r['implementation_sha256']==digest
    assert json.loads((LAB/'_browser_l077_results.json').read_text())['status']=='PASS'
    for name,sha in json.loads((LAB/'_sources_l077.json').read_text())['files'].items():
    ...
    checked=0
    with tempfile.TemporaryDirectory(prefix='l077-pages-') as tmp:
        ...
        for relative in [f'lessons/{SLUG}.html',f'reference/{SLUG}.html',f'labs/html/{SLUG}.html']:
            ...
            for tag in soup.find_all(['a','img','script','link']):
                ...
                checked+=1
    m=json.loads((ROOT/'lessons/manifest.json').read_text())
    assert next(x for x in m['lessons'] if x['id']==77)['labPath']==f'labs/{SLUG}.ipynb'
    result={'status':'PASS','student_todos':3,'portable_figures_per_notebook':4,'visible_canonical_definitions':len(expected),'live_todo_mutations_detected':mutations,'copied_pages_links':checked,'source_hashes':'PASS','full_synthetic_reproduction':'MATCH','live_colab':'NOT_CHECKED','deployment':'NOT_CHECKED'}
    (LAB/'_delivery_l077_results.json').write_text(json.dumps(result,indent=2)+'\n');print(result)
