...
from _solution_map_content import STORIES
...
def check():
    ...
    for n in range(48,71):
        ...
        assert len(s.select('.solution-seam'))==len(STORIES[n]['seams']),(n,'authored transitions missing')
        ...
        report.append(dict(lesson=n,maps=len(s.select('.solution-map')),authored_transitions=len(STORIES[n]["seams"]),code_and_saved_outputs='UNCHANGED'))
