...
for name,remote in paths.items():
 live=urllib.request.urlopen(root+remote,timeout=30).read();local=(P/'sources/l115'/name).read_bytes();assert live==local,name;verified[name]=hashlib.sha256(local).hexdigest()
r={'status':'PASS','commit':commit,'fresh_byte_verification':verified};(P/'_upstream_l115_results.json').write_text(json.dumps(r,indent=2));print(r)
