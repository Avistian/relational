...
**Built with TabPFN.** [Original model license](sources/foundation/v2-LICENSE). You implement six live operations for the paper's evaluation questions, then run the complete historical v2 weights. This is a key-parts evaluation mirror: the full pretrained network is provided visibly. The default run uses all CMC/Iris rows, split seed 42, both arms, all CMC held classes, and the generated IID/covariate/concept controls. The author-reference panel separately covers full CMC/red/white novelty and Iris/CMC/red feature tests with three seeds. CPU works; the ~29 MB immutable checkpoint and small released CSVs are downloaded or reused. No original pretraining, tuned seven-model benchmark, or paid/cloud run is performed.

**Workflow:** retrieve and predict before reading the results; implement the TODO; use CHECK to diagnose; run your actual namespace; then explain the EXIT evidence. Successful execution alone is not evidence of mastery. The complete recap below makes this notebook usable independently of the lesson.''')
