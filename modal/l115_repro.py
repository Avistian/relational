...
app=modal.App('l115-ogb-gcn')
image=(modal.Image.debian_slim(python_version='3.12')
 .pip_install_from_requirements(ROOT/'labs/requirements-l115-runtime.txt')
 .add_local_file(ROOT/'labs/relkit/patterns_l115.py','/work/relkit/patterns_l115.py')
 .add_local_file(ROOT/'labs/_source_check_l115.py','/work/_source_check_l115.py')
 .add_local_dir(ROOT/'labs/sources/l115','/work/sources/l115')
 .add_local_file(ROOT/'labs/data/l112/arxiv.zip','/data/arxiv.zip'))
volume=modal.Volume.from_name('l115-ogb-evidence',create_if_missing=True)
...
def run(seed,epochs,preset):
 ...
 from relkit.patterns_l115 import load_arxiv,normalized_adjacency,train_run
 from _source_check_l115 import check
 torch.set_num_threads(1);start=time.perf_counter()
 digest=hashlib.sha256(Path('/work/relkit/patterns_l115.py').read_bytes()).hexdigest()
...
def main(mode:str='pilot'):
 if mode=='pilot':print(pilot.remote())
  ...
  import json,hashlib
  budget=json.loads((ROOT/'labs/_budget_l115.json').read_text())
  assert budget['pilot_approved_for_full'] and budget['source_sha256']==hashlib.sha256((ROOT/'labs/relkit/patterns_l115.py').read_bytes()).hexdigest()
