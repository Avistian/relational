...
def run(preset='closer'):
    ...
    result['source_hashes']={p:hashlib.sha256(Path('/course/labs',p).read_bytes()).hexdigest() for p in ['relkit/temporal.py','relkit/temporal_v2.py','relkit/temporal_experiment_v2.py','relkit/tabm_v2.py']}
    output=Path('/results')/(preset+'-v2-results.json');output.write_text(json.dumps(result,indent=1));volume.commit()
