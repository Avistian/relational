...
- Source audit pinned tabular-benchmark revision 9d54cf53d9fd3159e367e70a00005f4fcbf2c79d. Classic-covariance smoother probabilities match within 4.45e-16; strict >.5 labels and common rotation match. Training uses robust covariance with an explicit eigenvalue floor. Added N(0,1) columns follow the figure caption, while released code matches training-column moments; this is disclosed.
- Local measurement: authors' processed January 2023 suite 337, electricity 44120, MagicTelescope 44125, bank-marketing 44126; 1800-row label-blind samples, split seed 51, training-only normal quantile transform, model seeds 0/1/2, intervention seed 51. Five conditions × three models × three tasks × three seeds = 135 fits. Fixed recipes, 20 epochs / 120 trees, not original search curves.
- Original mean ranks MLP 2.333, FT-T 2.000, XGBoost 1.667; Friedman p=.716531, CD=1.913624. Smoothing uses top-five raw labels as its baseline. Noise has finite-sample exceptions. Five condition tests are exploratory; noise p≈.0498 does not survive threshold .01 for five Bonferroni comparisons.
- Larger closer run completed: 6000 rows per task, 50 epochs / 400 trees, three seeds, all five conditions. Original means: electricity MLP .8003 / FT-T .8192 / XGBoost .8539; MagicTelescope .8506/.8481/.8578; bank marketing .7883/.8047/.8139. Original ranks 2.667/2.333/1.000, Friedman p=.09697. Remains INCOMPARABLE. Full resource preset, original search reorders and parameter sweeps remain NOT_RUN. Both Modal and live-code gated Colab operators supplied; no cloud job launched.
- Local experiment elapsed 118.6 seconds; larger experiment 2470.9 seconds with heavy workspace CPU contention. Do not use elapsed wall time here as a model-speed benchmark. Numerous stale read-only workspace-monitor git status workers were identified and stopped with approval; notebook and scale-up completion then progressed rapidly.
- Executed all teacher notebook cells with IPython in-process after the separate Jupyter kernel runner terminated/ran slowly. All 135 predictions/accuracies reconcile with reference. Stable identity cells were subsequently re-executed while preserving byte-identical trained code and outputs. Final notebook: 26 executed code cells, four student TODOs, six inline PNGs and prepared HTML; 58 local links checked.
- Actual live notebook gate: 45 smoke fits, then three-dataset resume without fitting; changed identity is rejected by the shared contract. Replaced marshal-based code hashes because Python reference bookkeeping changed after execution; stable bytecode/constants/default fingerprints now pass repeat execution. Cache boundaries are completed datasets, not epochs.
- Visuals: three independent widgets show smoother weights and hard threshold, invertible coordinates plus transported weights, and spurious noise split gains. This sharpens the older unconditional MLP-invariance/tree-immunity story. Twenty exported panels / 81 librsvg label boxes pass. Six notebook PNGs inspected. Browser and live Colab UI NOT_CHECKED; inline packaging is verified. Prepared HTML heading IDs normalized and copied relative lesson links rebased.
- Corrected shared glossary definitions, eight existing retrieval prompts and four paper cards while preserving their IDs. These are author corrections, not a new learner misconception diagnosis or mastery evidence. The review pool now separates representation equivalence from Adam training, and empirical robustness from immunity. Personal GLOSSARY.md remains unchanged.
...
### Easy to follow: assume nothing, intuition before formalism (2026-09-15, user directive) — applies to L048–L070 and all future lessons/labs

27. **Every lesson must be *easy to follow* for a reader meeting each concept fresh — clarity is a first-class deliverable, not a side effect of thoroughness (from 2026-09-15, user `/teach`: "make every lesson easy to follow and understand, it feels like they are assuming a lot of things").** Standard #17 (explain everything) was being met on paper — terms *were* defined — yet lessons still overloaded the reader: ~5 new concepts per paragraph, formalism before intuition, sentences stacking definition + caveat + claim, and honesty caveats constantly interrupting the explanatory thread. This standard fixes *cognitive load*, not missing definitions. The reference implementation is **L062** (approved by the user); its source `lessons/content/0062-tabpfn-v1.md` is the style template. Concretely, for every lesson from here on:
    - **Intuition before formalism.** Before any hard idea, equation, or derivation, give a short plain-language gloss, *then* the precise version. Use a marked aside: `> **In plain terms.** …` (markdown, L058–L070) or `<blockquote><p><strong>In plain terms.</strong> …</p></blockquote>` (raw HTML, L048–L057).
    - **One idea per sentence.** Break any sentence that stacks a definition + a caveat + a claim into separate sentences.
    - **Build big definitions/derivations in named beats.** Bold lead-ins (`**A prior.**`, `**Where the loop lands.**`) that walk a hard result step by step, rather than one dense block.
    - **Worked examples announced and numbers-first.** Lead concrete numeric passages with `**Worked example.**` and give the numbers before the generalization.
    - **Signpost long sections** with `###` sub-headings so the reader always knows what a chunk is for.
    - **Define every new term on first use, in its own short sentence** — do not assume recall from an earlier lesson (this reinforces #17, does not replace it).
    - **Separate honesty caveats from the teaching thread — WITHOUT weakening them.** Move scope/limitation/honesty facts (#20/#23/#25 discipline: `INCOMPARABLE`, `NOT_RUN`, `NOT_ESTABLISHED`, "does not reproduce", "the interval crosses zero", downscaling notes, "notebook completion alone is not mastery evidence", etc.) into `> **Scope check.** …` asides. Every such fact is preserved verbatim; it just no longer interrupts the explanation. **Exception:** when a "the model fails / accuracy drops / metric undefined" statement *is* the lesson's actual content (e.g. L069's failure modes), keep it in the main text.
    - **Aside CSS is in place:** `article blockquote` is styled in both `assets/lesson.css` (L048–L057) and `assets/foundation-course.css` (L058–L070); blockquote content is parsed as markdown, so bold/code/links work inside `> …` asides.
    - **Length may grow; never cut technical content or numbers to shorten.** This *extends* #17 and #26; it does not replace the thoroughness, first-principles, or honesty standards. Full recipe lives in the `lesson-pedagogy` skill. Retrofit target this pass: **L048–L070** (all 23 done, ~413 asides added).
