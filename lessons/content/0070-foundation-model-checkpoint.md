...
> **In plain terms.** A model's *name* proves nothing. What defends a result is the whole *procedure* around it: what data was available, how it was prepared, how the winner was chosen, and how it was scored. This checkpoint is about building and defending that procedure.

Imagine defending a relational model to a skeptical reviewer. You have a new neural architecture, a convincing diagram and a higher score than a tree baseline. The reviewer asks three questions. Which tree configurations did you try? Did the pretrained baseline see the same legal history? Did you choose your best experiment after looking at test labels? A **procedure** answers those questions: data availability → preparation → candidate fitting → validation selection → frozen prediction → evaluation. A model name alone cannot.
...
**What a downstream seed does and does not change.** A downstream seed means different things across arms. It changes neural initialization and minibatch order for TabM. It changes subsampling in XGBoost. It can change permutations and transformations around a fixed pretrained network. Three such seeds do not create three independently pretrained TabPFNs. The fixed split also leaves split uncertainty unmeasured.

> **Scope check.** The legacy arm called `TabM-mini` had an extra first-layer output adapter, per-member backbone biases and incorrect fan-in initialization. It remains an identifiable historical neural variant, but is not accepted as the paper's mini architecture. Corrected results are versioned, not pasted over old predictions. The fresh XGBoost fits form an additional **replay control** and a paired intervention control; they do not become a second XGBoost opponent in the primary rank table.

## Compare where each architecture spends its work

> **In plain terms.** These models "think" in different places. A tree or TabM stores the task in fitted weights; a pretrained in-context learner keeps a cross-task recipe frozen and reads your labeled rows at inference. Even a no-gradient `fit` can do real work.
...
**Corrected TabM-mini.** For a batch X with B rows and F features, expand to B×k×F, multiply by a first trainable member adapter R, and apply shared Linear → ReLU → Dropout blocks. Mini has no output adapter S and uses shared backbone biases. Independent heads return logits B×k×2. During training, average each member's cross-entropy; at inference, average member probabilities. **Worked example.** If two members give P(y=1)=0.9 and 0.1 on a positive row, the training loss is (−ln 0.9−ln 0.1)/2=1.204 nats, while the ensemble prediction is 0.5 with loss 0.693. Averaging logits or training only the averaged prediction changes the computation. This is the [TabM §3.3/§5.1](https://arxiv.org/html/2410.24210v3) distinction the provided model retains.
...
> **Scope check.** Ask the tutor to challenge your weakest link: row identity, validation independence, model fidelity, uncertainty, cost or transfer. Completion is a submitted and defended argument. Merely running the cells does not establish personal mastery.
