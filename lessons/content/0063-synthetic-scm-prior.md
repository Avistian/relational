...
> **In plain terms.** Before TabPFN ever meets your data, it was trained on a flood of *made-up* tables. This lesson opens the factory that stamps out those tables. Knowing the factory tells you which patterns the finished model is prepared to recognize — and which it will never have seen.

Start with a fact about real tables. The columns of a database table can depend on one another. A customer's age may affect income. Income may affect a purchasing decision. A recorded account score may be an effect of several hidden causes.
...
**The primary source.** The reference is [TabPFN v1, arXiv 2207.01848v6](https://arxiv.org/html/2207.01848v6), especially §4, Appendix C, Appendix E.4/Table 5 and the prior ablation in Appendix B.4/Table 4. This lab implements the central numerical **generator** path from [historical `mlp.py`, revision 44f60d8](https://github.com/automl/TabPFN/blob/44f60d83c545238c551f1481a5f6f031bbf376bf/tabpfn/priors/mlp.py), not a newly invented TabPFN predictor.
...
> **In plain terms.** There are two dice being rolled. One big roll picks the *rules of a world* — the graph, the coefficients, the noise settings. Many small rolls then generate *rows* inside that one world. Keeping these two levels straight is the single most important idea in the lesson.

**The pieces of a world.** A **structural causal model**, abbreviated SCM, contains variables, directed edges and structural assignments. A **directed edge** from variable i to variable j permits i to enter j's equation. A **directed acyclic graph**, or DAG, has no directed cycle. Because there is no cycle, its variables can be evaluated in an order where every parent is available before its child. A variable's **parents** are its immediate inputs. An **exogenous noise** variable is a source of randomness outside those assignments; independence of these noises is an assumption we choose for the synthetic world.

**What one assignment looks like.** In general, a structural assignment is `Z_j = f_j(Z_parents(j), ε_j)`. The deterministic function f_j and the noise distribution are both part of the world. Noise values ε_j differ between rows. The function, its parameters and the noise distribution stay fixed.

**Worked example.** Suppose `income = 2 × education + ε_income`. Every row uses coefficient 2. Individuals receive different draws of ε_income. Sampling a fresh coefficient for every individual would describe a different learning problem entirely.

**Naming the two levels.** Let φ denote all task-level choices: family, graph, weights, biases, activation, noise parameters, observed nodes and label rule. The sampling hierarchy is `φ ~ p(φ)` first, then many rows `D ~ p(D | φ)`. The PFN training algorithm draws new worlds repeatedly. Within a task, context rows and query rows teach and test the same relationship. They differ only in whether their labels are supplied to the predictor.

### One world per dataset, not per row

The distinction is easy to lose in code. A batch of 32 tasks needs 32 sampled worlds if the task contract specifies independent worlds. It does *not* mean 32 worlds per row. The released `new_mlp_per_example` flag chooses whether separate batch items share the sampled MLP, and the historical checkpoint records `True`. Our operator samples one new world per dataset, then vectorizes its equations over all rows.

<!--figure:generator-->

## The released generator: follow the computation all the way through

> **In plain terms.** The generator is a small neural network run *backwards from its purpose*: instead of learning weights to fit data, it fixes random weights and pushes noise through them to *manufacture* data. We now trace that push one layer at a time.

**The construction.** Appendix C.1 constructs a tractable subfamily of DAGs by starting with a layered MLP and deleting edges. A **multilayer perceptron**, or MLP, is a sequence of affine maps and elementwise nonlinearities. Here each scalar neuron is a graph variable. Connections occur between consecutive layers, so acyclicity is built into the representation. This construction covers many DAGs but not every conceivable DAG with every possible function.

**Reading the shapes.** For a layer with `d_in` parents and `d_out` children, the released weight matrix W has shape `d_out × d_in`: **child first, parent second**. For N rows, the parent matrix has shape `N × d_in`. Multiplying by `W.T` gives one weighted sum for every row and child. A bias b, shape `d_out`, shifts that sum. Broadcasting adds the same bias to all rows. The noise matrix ε, shape `N × d_out`, supplies a distinct noise value per row and child.

**The exact historical path**, step by step:
...
> **Scope check — paper versus executable source.** Appendix C.1 writes an abbreviated node equation with activation outside the weighted sum and noise. The historical `generate_module` instead builds `Activation → Linear → GaussianNoise`. These are not numerically interchangeable. This lesson's diagram, worked example, notebook and source checks follow the released order. The general SCM definition accommodates either order; a fidelity claim still has to choose one.

**Worked example — three affine layers, identity activation.** Take `C=(1,−2)`, `W0=diag(2,1)`, `b0=(0,1)`, giving `H0=(2,−1)`. Next use `W1=(1,−1)`, `b1=.5`, `ε1=.1`: `H1=2−(−1)+.5+.1=3.6`. Finally `W2=(2)`, `b2=−.2`, `ε2=.3` gives `H2=2×3.6−.2+.3=7.3`. Now switch the activation to tanh. The second value becomes `tanh(2)−tanh(−1)+.6≈2.3256`; it does *not* become `tanh(3.6)`, because activation acts on the *parents* before the affine map.

**How this is validated.** The lab checks this recurrence against the actual historical MLP class, using its sampled weights and recorded causes/noises. Sixteen cases span SCM/BNN, independent/block sparsity and Identity/Tanh/LeakyReLU/ELU.

> **Scope check.** The maximum absolute discrepancy is about `5.09×10⁻⁶` between the original float32 path and NumPy float64 evaluation. That checks conditional computation. It does not establish identical random streams, complete task distributions, or training trajectories.

## Sparsity is a task-level graph sample

> **In plain terms.** "Sparsity" here means deleting edges *once per world* to decide which variables are even connected. It is a choice about the shape of the world, not a training trick that changes every step.

**Why deleting an edge matters.** An edge has a coefficient. Setting that coefficient to zero removes its contribution from every row of the task. This is a **task-level graph sample**. It differs from ordinary neural-network dropout, which redraws masks during training to regularize a fixed predictive model.

### Independent-edge sparsity

Independent sparsification samples a binary keep mask with probability `1−p` per edge. The released checkpoint selects the square-root scaling flag. Its Bernoulli initializer uses the unusual divisor **`1−√p`**, not `√(1−p)`.

**Worked example.** For p=.25, raw weights `(1,2)` and mask `(1,0)` become `(2,0)`. We preserve that expression because changing it would change the released operator. The first affine matrix skips independent dropout and this correction.

### Blockwise sparsity

Blockwise sparsification uses a different rule. Divide a matrix into floor-sized diagonal blocks. Keep those blocks. Leave other entries and any leftover rows/columns zero. Divide kept raw weights by `√keep_fraction`.

**Worked example.** For a 4×4 matrix with two 2×2 blocks, half the entries remain and their scale factor is √2. This branch also acts on the first affine matrix. It can create strongly interacting groups with few paths between groups. Neither rule guarantees that each selected feature predicts the selected target.

<!--figure:sparsity-->

### Why prefer smaller, sparser worlds?

The paper's **simplicity prior** favors mechanisms with fewer nodes and parameters. This implements one particular meaning of Occam's razor. It is an inductive assumption, not a proof that a smaller graph generated your database.

> **Scope check.** A deep nonlinear graph can generate a simple observed problem. A small hidden-variable graph can generate a difficult one. Count parameters and inspect the observation rule before equating generator size with prediction difficulty.

## Hyperparameters are themselves random variables

> **In plain terms.** In an ordinary project, a hyperparameter is a knob *you* tune on *your* dataset. Here every knob is itself sampled, because the goal is to describe a whole *distribution of tasks*, not to fit one.

**What a hyperparameter controls here.** The paper mixes a BNN generator and an SCM generator with equal probability. Inside each family it varies graph depth, width, activation, sparsity, noise and observation choices. A hyperparameter here controls a **distribution of tasks**. It is not a setting fitted separately by cross-validation on the deployment table.

**How positive quantities are sampled.** Table 5's shorthand TNLU samples positive values in stages: draw distribution parameters on a logarithmic scale, sample a positive truncated normal value, optionally round, then add a minimum. A logarithmic scale gives comparable probability to multiplicative ranges such as .01–.1 and .1–1. Truncation prevents negative scales or sizes.

> **Scope check — the sampler is not exactly Table 5.** The historical implementation's `meta_trunc_norm_log_scaled` draws a mean μ and a coefficient of variation r, then sets standard deviation `σ=μr`. Its r range defaults to .01–1. Table 5 describes independently log-uniform μ and σ. Those descriptions are not the same distribution.

**The full set of ranges and our deliberate deviations.** Each row below pairs the paper/checkpoint range with the choice this numerical lab makes.
...
### The class-count sampler and what the lab keeps

The source class-count sampler chooses binary half the time; otherwise it rounds a uniform draw from 2 to the maximum class count. Thus the remainder is not exactly uniform over integers: the endpoint bins are narrower. The lab retains that rule with maximum 10, samples bounds from the whole synthetic episode and applies one class permutation to every row. It does not force balanced classes.

> **Scope check.** The full historical wrapper additionally tries to repair context/query class support, remaps valid class IDs contiguously and applies a random label shift. The lab reproduces the inner rank operator and shared relabeling, not those later wrapper steps. Using full-episode targets for bound construction is a declared **joint task generator**, as in the paper; it is not a recipe for choosing real test-label thresholds. Likewise, fitting preprocessing on synthetic episode rows in the original wrapper must not be copied uncritically into real deployment evaluation. Here the diagnostic probe fits its scaler and coefficients only on context rows. The fixed synthetic label rule is defined before the learner receives context/query roles.

## The query's features can also update beliefs about the world

> **In plain terms.** You might think only the *labeled* rows tell you which world you are in. But because the generator models the feature distribution too, even an *unlabeled* query row's features are a clue about which world produced this table.

**The quantity we want.** A posterior predictive distribution, or **PPD**, averages predictions over worlds weighted by how plausible those worlds remain after observing data. Let D be labeled context, x* a query feature vector and y* its unknown label. In a joint generative SCM:

`p(y* | x*, D) = ∫ p(y* | x*, φ) p(φ | x*, D) dφ`.

**Why the query's features enter.** Bayes' rule gives `p(φ | x*, D) ∝ p(φ) p(D | φ) p(x* | φ)` when rows are conditionally independent given φ. The last term, `p(x* | φ)`, shifts the posterior: a query's features can favor one world over another even before its label is known. The familiar simplification using only `p(φ | D)` is justified when the feature distribution contributes the same factor across hypotheses, or when the prior is explicitly conditional on fixed inputs. A general SCM models X as well as Y, so that simplification is not automatic. Joint synthetic label construction can require an even more explicit episode-level likelihood; the independent-row factorization below belongs to the stated finite oracle.

**Worked example — two candidate worlds.** Prior weights are `(.5,.5)`. Context likelihoods are `(.4,.6)`. Query-feature likelihoods are `(.2,.8)`. Their unnormalized posterior masses are `(.04,.24)`. Divide by the total .28 to obtain `(1/7,6/7)`. If the class-1 probabilities in those worlds are `(.1,.9)`, the PPD is `11/14≈.7857`. Ignoring query-feature evidence gives context-only weights `(.4,.6)` and prediction .58. The difference is inference about a **distribution of worlds**, not selecting a single causal graph.

<!--figure:posterior-->

### Doing this arithmetic safely
